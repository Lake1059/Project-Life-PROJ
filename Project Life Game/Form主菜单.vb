Imports System.ComponentModel

Public Class Form主菜单
    Private Sub Form主菜单_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Form控制台.Visible = False
    End Sub

    Private Sub Form主菜单_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing

    End Sub

    Private Sub Form主菜单_Shown(sender As Object, e As EventArgs) Handles Me.Shown

    End Sub

    Private Sub Form主菜单_SizeChanged(sender As Object, e As EventArgs) Handles Me.SizeChanged

    End Sub

    Private Sub UiButton1_Click(sender As Object, e As EventArgs) Handles UiButton1.Click

    End Sub

    Private Sub UiButton2_Click(sender As Object, e As EventArgs) Handles UiButton2.Click

    End Sub

    Private Sub UiButton3_Click(sender As Object, e As EventArgs) Handles UiButton3.Click

    End Sub

    Private Sub UiButton4_Click(sender As Object, e As EventArgs) Handles UiButton4.Click

    End Sub

    Private Sub UiButton5_Click(sender As Object, e As EventArgs) Handles UiButton5.Click
        If Form控制台.Visible = False Then Form控制台.Show()
    End Sub
End Class
